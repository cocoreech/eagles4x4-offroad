...
**Distinct from:** the guest-**Booking** contact list used for catalog-announcement emails (new event/promo/product/service/build). That's a separate, informal notion — queried live from `bookings.contact_email` where `customer_id IS NULL`, no table of its own — because it doesn't need conversion tracking, just a mailing target. Both are "people we know about but who haven't signed up," but only the chat-captured kind gets a row.
...
**Key decision:** the Concierge is extended to anonymous homepage/site-wide visitors (no account) via a chat widget — same grounding and escalation rules as the Inbox version, not a separate/simpler bot. A guest's conversation persists anonymously (browser session-scoped), not tied to any account.

**Key decision:** a guest chat is claimed into an account **only when the signup happens in the same browser** — matched on the guest session cookie, deliberately NOT on email. On that match the guest's messages are migrated into the new account's Inbox thread (the guest record is then deleted; any captured **Lead** is marked converted). Cookie-matching, not email-matching, is the whole point: email-matching would attach a shared/public computer's guest chat to whoever signs up next. This differs from a guest **Booking**, which *does* link by email — a booking is the person's own verified contact info, a chat is not. If the guest signs up from a different browser, or clears cookies first, the chat does not carry over (it stays anonymous; the new account's Inbox starts empty).
